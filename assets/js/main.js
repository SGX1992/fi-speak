import { Poster } from './poster.js?v=ee1512a';
import { EDITIONS, DEFAULT_EDITION, NO_EDITION, byId, pickerLabel, prettyDate } from './editions.js?v=ee1512a';
import { canShareFiles, download, hasMp4, shareFile, toGif, toMp4, toPng, toWebm } from './exporters.js?v=ee1512a';
import { backgroundVideo, clips, posterUrl } from './videobg.js?v=ee1512a';
import { variants, thumbUrl } from './backgrounds.js?v=ee1512a';
import { combo } from './combo.js?v=ee1512a';
...

{
  ...
  none.textContent = 'Choose your event';
  editionInput.append(none);
}
for (const e of EDITIONS) {
  const o = document.createElement('option');
  o.value = e.id;
  o.textContent = `${pickerLabel(e)} — ${prettyDate(e)}`;
...
const editionCombo = combo({
  ...
  items: [
    { value: NO_EDITION, title: 'Choose your event', meta: '' },
    ...EDITIONS.map((e) => ({ value: e.id, title: pickerLabel(e), meta: prettyDate(e) })),
  ],
});
...
/* Text is measured with the real font, so nothing may render before the webfont
   ...
   Poppins is self-hosted and ships with no operating system, so unlike a system
   face there is no version of this that resolves instantly: it is always a
   fetch, and always worth waiting for. */
const fontsReady = Promise.all([
  document.fonts.load('700 150px "Poppins"'),
  document.fonts.load('600 44px "Poppins"'),
  document.fonts.load('500 34px "Poppins"'),
  document.fonts.load('400 60px "Poppins"'),
]).catch(() => {});
...
/* ---------- exports ---------- */
function slug() {
  const s = (nameInput.value.trim() || 'footprint').toLowerCase().replace(/[^a-z0-9]+/g, '-');
  return s.replace(/^-|-$/g, '') || 'footprint';
}
const filename = (ext) =>
  `fi${editionInput.value ? '-' + editionInput.value : ''}-${variant().slug}-${slug()}.${ext}`;
